...
function EmptyCommunicator() {}
EmptyCommunicator.prototype.buildUrl = (servlet, params) => servlet + '?' + params.join('&');

describe("SolrJsX:", function () {
	// Now - GO with the tests.
	describe("Configuration:", function () {
		var Configurator = a$(Solr.Configuring);

		it("Adds/Gets raw parameter", function () {
			var main = new Configurator();
		...
		it("Adds/Gets single-parameter", function () {
			...
			expect(main.getParameter("q")).toEqual({
				name: "q",
				value: "field2:value2"
			});
		});
		...
		it("Adds/Gets multi-parameter", function () {
			...
			expect(main.getParameter("fq")).toEqual([{
				name: "fq",
				value: "field1:value1"
			}, {
				name: "fq",
		...
		it("Removes multi-parameter", function () {
			...
			expect(main.getParameter("fq", 0)).toEqual({
				name: "fq",
				value: "field2:value2"
			});
		});
	...
	var UrlManager = a$(Solr.Configuring, Solr.QueryingURL, EmptyCommunicator),
		JsonManager = a$(Solr.Configuring, Solr.QueryingJson, EmptyCommunicator);
	
	describe("Making URL-based queries", function () {

		var main = new UrlManager({
			servlet: 'test'
		});
		...
		it("Forms the URL on simple parameters", function () {
			...
			main.addParameter("rows", 20);
			expect(main.prepareQuery()).toEqual({
				url: "test?q=f%3Av&rows=20"
			});
		});
		...
		it("Forms the URL on multi parameters", function () {
			...
			expect(main.prepareQuery()).toEqual({
				url: "test?q=*%3A*&fq=field1%3Avalue1&fq=field2%3Avalue2&json.nl=map"
			});
		});
	...
	describe("Json API faceting", function () {
		...
		describe("Simple faceting", function () {
			...
			it("Property adds a simple value", function () {
				main.resetParameters();
				facet.addValue("foo");
				expect(main.getParameter('json.filter')).toEqual([{
					name: 'json.filter',
					value: "field:foo"
				}]);
			});
		...
		describe("Patterned faceting with multi-values and aggregation", function () {
			...
			it("Property adds a simple value", function () {
				...
				expect(main.getParameter('fq')).toEqual([{
					name: 'fq',
					value: "-(condition:yes OR -field:foo)"
				}]);
			});
			...
			it("Property add more values a value", function () {
				...
				expect(main.getParameter('fq')).toEqual([{
					name: 'fq',
					value: "-(condition:yes OR -field:(foo bar))"
				}]);
			});
	...
	describe("Patterning abilities", function () {
		var main = new UrlManager();
		var range = new(a$(Solr.Ranging, Solr.Patterning))({
			id: "test",
			field: "field",
			valuePattern: "-(condition:yes OR -{{v}})"
		});
		...
		it("Property adds a simple range", function () {
			...
			expect(main.getParameter('fq')).toEqual([{
				name: 'fq',
				value: "-(condition:yes OR -field:[3 TO 4])"
			}]);
		});
		...
		it("Property overrides a value", function () {
			...
			expect(main.getParameter('fq')).toEqual([{
				name: 'fq',
				value: "-(condition:yes OR -field:[1 TO 5])"
			}]);
		});
	});
});